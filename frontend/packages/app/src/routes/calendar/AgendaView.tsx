...
/** Number of days shown in the Agenda list, starting from the selected date (inclusive). */
export const AGENDA_VIEW_DAYS = 5;
...
/** A rolling agenda list — the selected date plus the next `AGENDA_VIEW_DAYS - 1` days — rather
 * than a calendar-aligned Sun–Sat week, since the block-style schedule grid replaced Week for that. */
export function AgendaView({ date, onViewEvent, onAddForDay }: AgendaViewProps) {
  const [filter, setFilter] = useState<Set<string>>(new Set());
  const from = startOfDay(date);
  const to = addDays(from, AGENDA_VIEW_DAYS);
  const { data: events } = useEvents(from, to);
  const today = new Date();

  const days = Array.from({ length: AGENDA_VIEW_DAYS }, (_, i) => addDays(from, i));
        ...
        {days.map((day) => {
          ...
          return (
            <div key={day.toISOString()}>
              {/* Single tap on the day label adds an entry for that day — no double-tap needed
                  here, same reasoning as the old Week list (see plan_001.md decision #10). */}
              <button
